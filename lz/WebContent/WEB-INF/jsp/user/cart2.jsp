						...
						<c:if test="${cartList!=null }">
							<c:forEach items="${cartList }" var="GoodsItem">
								<tr id="${GoodsItem.bookId}">
									<td><a href="${pageContext.request.contextPath }/book/bookInfo/${GoodsItem.bookId+1}"> <img
									...
									<td><a href="javascript:void(0)"
										...
										<span name="bookNum">${GoodsItem.bookNum }</span>
											...
											name="${GoodsItem.bookId}" onclick="subGoods(this.name)">-</a></td>
									<td ><span id="subtotal_money${GoodsItem.bookId}">${GoodsItem.bookNum*GoodsItem.currentPrice }</span>￥</td>
									<td><a href="${pageContext.request.contextPath }/cart/delete/${item.bookId}">删除<a>
									</td>
								</tr>
							</c:forEach>
						</c:if>
